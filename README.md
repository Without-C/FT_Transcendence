...
<img src="42logo.webp" alt="배너" width="100%"/>
</a>
...
- 프로젝트 이름: Without C
- 프로젝트 설명: Ping-Pong 게임 구현 및 유저 관리
...
- **로그인**:
  ...
  - 42 계정과 구글 계정으로 로그인할 수 있습니다.  

- **pong게임**:
  - 유저가 키보드를 사용하여 pong게임을 플레이 합니다.
  - 토너먼트:
    - 유저들이 차례대로 게임을 진행하며 현재 누구와 누가 게임중인지, 플레이어의 순서가 어떤지 보여집니다.
    - 토너먼트 처음에 각 유저는 이름을 입력해야한다.
  - 원격 접속으로 유저간 1대1 경기도 가능합니다.

- **Block chain 적용**:
  - 토너먼트 점수를 블록체인에 안전하게 저장

- **유저 관리**:
  - 인트라 프로필을 이용하여 유저의 기본 정보를 표시합니다.  
  - 유저는 다른 사람들을 친구로 추가하고 온라인 상태를 볼 수 있어야한다.  
  - 유저 프로필에서는 이겼는지, 졌는지 stat이 나와야한다.  
  - 모든 유저는 Match History가 있어야한다.

- **2FA 기능 구현**:
  - JWT를 사용하여 안전하게 인증, 인가하고, 유저 세션과 리소스에 접근하는 것을 안전하게 관리
  - email-based verification을 사용

- **Microservices로 디자인**:
  - 벡엔드를 작고, loosely-coupled하게, 각 기능에 대해 각각 책임을 가지도록 나누었다.
  - microservice간 RESTful API나 message queue를 사용하여 data exchange를 하도록 구현
  - flexibility, scalability, maintainability를 높이는 것에 주안점을 두었습니다.

- **접근성 향상**:
  - 모든 타입의 기기에서 seamless하게 동작
  - 새로운 브라우저에서도 잘 작동
  - 넓은 사용자를 수용하기위해서 최소 세개의 언어를 지원
 
- **Server-Side Pong 구현**:
  - 공 움직임, 점수 카운팅, 유저 인터랙트 로직이 server-side에서 동작함
  - Pong game을 web application과 통합
...
|  |  |  |
|-----------------|-----------------|-----------------|
| 장재열    |  <img src="https://github.com/user-attachments/assets/c1c2b1e3-656d-4712-98ab-a15e91efa2da" alt="이동규" width="100"> | <ul><li>프로젝트 계획 및 관리</li><li>팀 리딩 및 커뮤니케이션</li><li>커스텀훅 개발</li></ul>     |
| 최시영    |  <img src="https://github.com/user-attachments/assets/78ec4937-81bb-4637-975d-631eb3c4601e" alt="신유승" width="100">| <ul><li>메인 페이지 개발</li><li>동아리 만들기 페이지 개발</li><li>커스텀훅 개발</li></ul> |
| 이소민    |  <img src="https://github.com/user-attachments/assets/78ce1062-80a0-4edb-bf6b-5efac9dd992e" alt="김나연" width="100">    |<ul><li>홈 페이지 개발</li><li>로그인 페이지 개발</li><li>동아리 찾기 페이지 개발</li><li>동아리 프로필 페이지 개발</li><li>커스텀훅 개발</li></ul>  |
| 조영호    |  <img src="https://github.com/user-attachments/assets/beea8c64-19de-4d91-955f-ed24b813a638" alt="이승준" width="100">    | <ul><li>회원가입 페이지 개발</li><li>마이 프로필 페이지 개발</li><li>커스텀훅 개발</li></ul>    |
| 신영기    |  <img src="https://github.com/user-attachments/assets/beea8c64-19de-4d91-955f-ed24b813a638" alt="이승준" width="100">    | <ul><li>회원가입 페이지 개발</li><li>마이 프로필 페이지 개발</li><li>커스텀훅 개발</li></ul>    |
